...
VOID				WriteText(HDC hdc, RECT clientRect, TABLE table, INT columnWidth, INT columnHeight);
VOID				WriteRow(HDC hdc, RECT clientRect, TABLE table, INT rowIndex, INT columnWidth, INT columnHeight);
	...
	switch (message){
		...
		case WM_COMMAND:
			if (LOWORD(wParam) == IDOK){			
				...
				if (cchColums == 0) {
					MessageBox(hDlg, L"Выберите другой файл",	L"Error", MB_OK);
				...
				*((LPWORD)lpstrColums) = cchColums;
				...
				if (table.numOfColums == 0){
					MessageBox(hDlg, L"Как вы себе это представляеете? Колонок должно быть больше 0", L"Error", MB_OK);
					return (INT_PTR)FALSE;
				}
				if (table.numOfColums > MAX_COLUMNS) {
					MessageBox(hDlg, L"Максимальное количество колонок - 10", L"Error", MB_OK);
					return (INT_PTR)FALSE;
				}
...
{	
	RECT clientRect;
	GetClientRect(hWnd, &clientRect);
	int CharHeight = clientRect.bottom / 30;
	if (CharHeight<15){
		CharHeight = 15;
	}
	HPEN pen = CreatePen(PEN_STYLE, PEN_WIDTH, PEN_COLOR);
	HGDIOBJ oldPen = SelectObject(hdc, pen);
	HFONT font = CreateFont(CharHeight, CharHeight/2, 0, 0, FW_NORMAL, false, false, false, 0, 0, 0, 0, 0, NULL);
	HGDIOBJ oldFont = SelectObject(hdc, font);

	int columnWidth = clientRect.right / table.numOfColums;
	int x = 0;
	int columnHeight = clientRect.left / table.numOfRows;

	WriteText(hdc, clientRect, table, columnWidth, columnHeight);
...
VOID WriteText(HDC hdc, RECT clientRect, TABLE table, INT columnWidth, INT columnHeight)
{
	for (int i = 0; i < table.numOfRows; i++) WriteRow(hdc, clientRect, table, i, columnWidth, columnHeight);
}
